...
import { ArrowUpRight, CalendarDays, ChevronRight, Menu, X } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="workspace">
    ...
    <aside id="workspace-navigation" className={`sidebar ${open ? "is-open" : ""}`} ref={drawer} aria-label="Workspace navigation" role={open ? "dialog" : undefined} aria-modal={open || undefined}>
      <div className="brand"><div className="brand-lockup"><Link href="/" onClick={() => setOpen(false)} aria-label="ScaleSight home"><BrandLogo /></Link><p>Managed Beverage Intelligence</p></div><button className="mobile-only icon-button" aria-label="Close navigation" onClick={() => { setOpen(false); toggle.current?.focus(); }}><X size={20}/></button></div>
